...
def save_image_array_as_png(image, output_path):
    #Save an image (represented as a numpy array) to PNG.
    image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
    with tf.gfile.Open(output_path, 'w') as fid:
        image_pill.save(fid, 'PNG')

def encode_image_array_as_png_str(image):
    #Encode a numpy array into a PNG string.
...
def draw_bounding_box_on_image_array(image,
    ...
    #Add a bounding box to an image (numpy array)
    image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
...
def draw_keypoints_on_image_array(image,
                                  ...
                                  radius=2,
                                  use_normalized_coordinates=True):
    #Draw keypoints on an image(numpy array)
    image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
...
def draw_keypoints_on_image(image,
    ...
    if use_normalized_coordinates:
        keypoints_x = tuple([img_width*x for x in keypoints_x])
        keypoints_y = tuple([img_height*y for y in keypoints_y])
    for keypoints_x, keypoints_y in zip(keypoints_x, keypoints_y):
        draw.ellipse([(keypoints_x - radius, keypoints_y - radius),
                      (keypoints_x + radius, keypoints_y + radius)],
                       outline=color, fill=color)

def draw_mask_on_image_array(image, mask, color='red', alpha=0.7):
    #Draw mask on an image
